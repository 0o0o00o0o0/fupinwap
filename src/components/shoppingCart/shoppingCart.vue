    ...
    <div class="hauto">
      <div class="shopping-header clearfix">
        ...
        <div
          class="manage"
          v-show="haveData"
          @click="operateStatus=!operateStatus"
        >管理</div>
        <div
          class="manage"
          v-show="operateStatus"
          @click="operateStatus=!operateStatus"
        >完成</div>
      </div>
      <div
      ...
      >
        ...
        >
          ...
          <div class="content">
            <div class="title">{{item.commodityInfo.commodityTitle}} {{item.commodityInfo.specificationName}}</div>
            <div class="flex">
              <div class="price">
                <!-- <img
                  ...
                  alt=""
                > -->
                ￥{{ item.commodityInfo.displayPrice}}</div>
              <div class="number">
                <van-stepper
                  v-model="item.commoditySize"
      ...
      <div v-show="haveData" class="opreate-bar flex">
        <div class="checkbox-all clearflex">
          <van-checkbox
            v-model="chkAll"
            checked-color="#07c160"
            @change="handleClickChkAll"
          ><span class="f12">全选</span></van-checkbox>
        </div>
        <div
          class="flex"
          v-show="!operateStatus"
        >
          <div class="price-wrap"><span class="label">合计:&emsp;</span><span class="price-icon">
            <!-- <img
                src="../../assets/images/fu.png"
                alt=""
              > -->
             ￥ <span class="price">{{totalPrice | toFixedFilter}}</span></span></div>
  ...
  methods: {
    ...
    goToBalance() {
      ...
      sessionStorage.setItem(
        "BUY_CARTLIST_INFO",
        JSON.stringify(validCartList)
      );
      this.$router.push({
        name: "confirmOrder",
        query: { pageSource: "cart" }
      });
    },
    handleClickCheckbox(item) {
      if (item.checked) {
        this.totalPrice +=
          parseFloat(item.commodityInfo.displayPrice) *
          parseInt(item.commoditySize).toFixed(2);
      } else {
        this.totalPrice -=
          parseFloat(item.commodityInfo.displayPrice) *
          parseInt(item.commoditySize).toFixed(2);
    ...
    handleChangeProductNumber(item) {
      ...
      this.$axios
        ...
        .then(json => {
          ...
          self.getTotalPrice();
          this.storeNumers[item.commodityInfo.id] = item.commoditySize;
    ...
    handleClickDelete() {
      for (let i = 0; i < this.cartList.length; i++) {
        if (this.cartList[i].checked) {
          this.handleDeleteProductById(this.cartList[i]);
        }
      }
    },
    handleDeleteProductById(item) {
      var self = this;
      var params = {};
      params.id = item.id;
      ...
      this.$axios
        ...
        .then(json => {
          delete this.storeNumers[item.commodityInfo.id];
          localStorage.setItem("storeNum", JSON.stringify(this.storeNumers));
          self.getCartData();
          bus.$emit("updateShoppingNumber", "");
        })
