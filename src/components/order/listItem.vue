  ...
  >
    <div class="order-item">
      <div class="status-wrap clearfix">
        <!-- <span>,剩余20:19</span> -->
        <span class="order-no"><span>订单号:</span>{{item.orderReference}}<div class="order-time">{{item.createTime}}</div></span>
        <span
          class="status"
          v-show="endTimeStr"
        >{{endTimeStr}}</span>
      ...
      >
        ...
        <div class="clearflex text-right">
          <div class="total-price">
            ...
            ￥<span>{{item.actualPayment}}</span></div>
      ...
      <div class="footer">
        ...
        <button
          ...
          @click="$router.push('/evaluat?'+'&itemId='+item.id+'&openId='+openId+'&productId='+item.shoppingItemList.map(it=>it.commodityId).join(','))"
        >去评价</button>
        <div style="color:orange;padding: 7px 10px;" v-show="item.orderStatus == 4">交易完成</div>
        <!-- <button
              type="button"
              class="pay-btn"
              v-show="item.orderStatus == 0"
