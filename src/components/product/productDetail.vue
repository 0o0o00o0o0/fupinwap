  ...
  >
    <div class="hauto">
      ...
      <div
        ...
        style="background-color:rgb(46,126,94);height:2rem;color:white;line-height:2rem;text-align:center;display:flex;align-items:baseline;justify-content: space-evenly;"
      >
        <i style="font-size:1.3rem;;font-weight:600">预售</i> <span>发货时间：团购预售时间截止后即刻发货</span>
      </div>
      <div class="product-info-wrap">
        <div class="title">{{productInfo.commodityTitle}}<span style="margin-left:.5rem;color:#b4b4b4;font-size:.8rem">({{productInfo.specificationName}})</span>
          <!-- <span class="f12">{{productInfo.specificationName}}</span> -->
        </div>
        <!-- <div><span class="sub-title">{{productInfo.commoditySubtitle}}</span></div> -->
        <div
          class="price-wrap flex"
          style="    align-items: center;"
        >
          <span class="price">￥<span>{{productInfo.displayPrice}}</span><del v-show="productInfo.originPrice">￥{{productInfo.originPrice}}</del> <span style="font-size:.8rem;margin-left:1rem">仅剩{{productInfo.stock}}份</span></span>
          <span class="private">限时供应，精品优选</span>
        </div>
        <div class="setting">
          <div class="item"><span>所属区域:</span><span> {{productInfo.commodityPlace}}</span></div>
          <div class="item"><span>支持配送地区:</span><span> {{productInfo.supportPlace}}</span></div>
        </div>
      ...
      <div
        @click="$router.push({name:'afterSale'})"
        class="com-pri"
      >
        售后服务
        <span>查看详情
        ...
        >
          <div class="flex shopping-nav">
            ...
            >
              ...
              >{{totalNumber}}</span></div>
            <div><span
                class="buy"
                @click="addShoppingCart('body')"
              >加入购物车</span></div>
          </div>
          <div class="clearflex h100"><button
              type="button"
              class="cart-btn"
              style="background-color:rgb(211, 55, 42);color:white"
              @click="goToBuy('body')"
            >立即购买</button><!-- <button type="button" class="cart-btn buy-bg" @click="goToBuy">立即购买</button> -->
          </div>
        </div>
        <div
          v-else
          class="h100 flex shopping-nav"
        >
          <div
            class="clearflex shopping-cart"
            @click="goToShoppingCart"
      ...
      <div class="spec-popup">
        ...
        <div class="header flex">
          <div class="clearflex image">
            <img
              :src="imgServer+specInfo.coverImage"
              alt=""
            >
          </div>
          <div>
            ...
            <div class="mh15 flex">
              <div class="price-wrap">￥{{specInfo.displayPrice}} <span class="f-gray">*{{specInfo.number}}</span></div>
              <div class="price-wrap clearflex">￥{{specInfo.totalPrice}}</div>
            </div>
          </div>
        </div>
        <div class="spec-list">
  ...
  methods: {
    ...
    goToBuy(type) {
      if (type == "popup") {
        ...
        this.$router.push({
          name: "confirmOrder",
          query: { pageSource: "buy", productId: this.productId }
        });
      } else {
        if (this.productInfo.specificationFlag == 1) {
          this.specPopup = true;
