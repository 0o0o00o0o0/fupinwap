    ...
    <div class="hauto">
      <div class="product-list" v-show="haveData">
        ...
        >
          ...
          <div class="shui-ying" v-show="item.boardStatus == 3">
            <img src="../../assets/images/ing.png" alt="" />
          </div>
          <div class="shui-ying" v-show="item.boardStatus == 4">
            <img src="../../assets/images/end.png" alt="" />
          </div>
          <div
            class="shui-ying"
            v-show="item.boardStatus == 2 || item.boardStatus == 1"
          >
            <img src="../../assets/images/startBefore.png" alt="" />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    goToProductList(item, index) {
      if (item.boardStatus == 4) {
        ...
        sessionStorage.setItem(
          "FESTIVAL_IMG_URL",
          this.imgServer + item.imageUrl
        );
        this.$router.push({
          name: "productList",
          query: {
            labelNamelist: item.imageName,
            index: index,
            festivalType: 2,
          },
        });
    ...
    mygets() {
      let redirect_urls = encodeURIComponent("http://wap.chhtf.com/#/festival");
      let urls =
