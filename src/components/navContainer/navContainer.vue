<template>
  <div class="">
    ...
    <van-tabbar id="navContainer" v-model="active" @change="handlerChange">
      ...
      <van-tabbar-item @click="handlerChange(10)" icon="apps-o">
        {{  "全部分类" }}
      </van-tabbar-item>
      <van-tabbar-item
        icon="cart-o"
        :info="cartProNumber"
        v-if="cartProNumber > 0"
      >
        购物车
      </van-tabbar-item>
      <van-tabbar-item icon="cart-o" v-else> 购物车 </van-tabbar-item>
...
export default {
  ...
  mounted: function () {
    ...
    if (
      sessionStorage.getItem("ACTIVE_MENU_INDEX") &&
      sessionStorage.getItem("ACTIVE_MENU_INDEX") != "null" &&
      sessionStorage.getItem("ACTIVE_MENU_INDEX") != "undefined"
    ) {
  ...
  methods: {
    handlerChange(index) {
      ...
      if (index == 0) {
      sessionStorage.getItem('temporary') ? self.$router.push({ name: "temporary" }):self.$router.push({ name: "fupin" });
      } else if (index == 10) {
        // this.secondType = this.secondType ? 0 : 1;
        // const name = this.secondType ? "index" : "fupin";
    ...
    // mygets() {
    //   let redirect_urls= encodeURIComponent("http://wap.chhtf.com/#/navContainer/inde");
